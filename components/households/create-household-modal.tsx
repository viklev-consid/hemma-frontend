...
/**
 * Intercepted-route modal wrapper around the shared `<CreateHouseholdForm/>`.
 *
 * Behaviour:
 * - `onSuccess` — close the modal locally and replace the intercepted
 *   URL with the newly-created household route. The form will already have
 *   invalidated /my.
 ...
 *
 * Hard-refresh on `/app/households/new` bypasses the slot entirely
 * and renders the standalone page at `app/(app)/app/households/new/`.
 */
  ...
  return (
    ...
    >
      <DialogContent className="max-w-xl">
        ...
        <DialogTitle className="sr-only">Create household</DialogTitle>
        <CreateHouseholdForm
          variant="plain"
          onSuccess={(data) => {
            setOpen(false);
