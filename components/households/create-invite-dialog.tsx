...
import {
  createHouseholdInvitationMutation,
  listHouseholdInvitationsQueryKey,
} from "@/api/generated/@tanstack/react-query.gen";
import type { CreateHouseholdInvitationResponse } from "@/api/generated";
import {
  handleProblem,
  mapProblemToFieldErrors,
  type ProblemDetails,
} from "@/api/problems";
import { RawTokenPanel } from "@/components/households/raw-token-panel";
...
import { formatRoleLabel, rolesBelow } from "@/lib/household-roles";
...
function CreateInviteContent({ onClose }: { onClose: () => void }) {
  const t = useTranslations("households.invitations.create");
  const tCommon = useTranslations("common.actions");
  const household = useHousehold();
  const queryClient = useQueryClient();

  const callerRole = household.role ?? "";
  const candidateRoles: HouseholdRole[] = callerRole ? rolesBelow(callerRole) : [];

  const [email, setEmail] = useState("");
  const [role, setRole] = useState<string>(candidateRoles[0] ?? "");
  const [fieldErrors, setFieldErrors] = useState<Record<string, string>>({});
  const [result, setResult] =
    useState<CreateHouseholdInvitationResponse | null>(null);
    ...
    <>
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          mutation.mutate({
            path: { householdRef: household.slug },
            body: { email: trimmed, role: role as HouseholdRole },
          });
        }}
      >
        <FieldGroup>
