...
import { RoleChangeDialog } from "@/components/households/role-change-dialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { roleRank } from "@/lib/household-roles";

const columnHelper = createColumnHelper<HouseholdMemberItem>();
...
export function MembersTable() {
  const t = useTranslations("households.members.table");
  const tActions = useTranslations("households.members.actions");
  const household = useHousehold();
  const { currentUser } = useAuth();
  const canManage = useCanInActiveHousehold(HOUSEHOLD_PERMISSION.MembersManage);
  ...
  // this with Households.Owner.LastOwnerRequired — we just don't let
  // the user click a button that's guaranteed to fail.
  //
  // TODO(household-pagination): this count assumes the members listing is
  ...
  // list response (or a dedicated `GET /households/{ref}/owners/count`
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("role", {
        ...
        cell: ({ getValue }) => <HouseholdRoleBadge role={getValue()} />,
      }),
...
function MemberCell({ member }: { member: HouseholdMemberItem }) {
  const t = useTranslations("households.members.table");
...
function RemoveMemberDialog({
  ...
  const t = useTranslations("households.members.remove");
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      if (isSelf) {
        ...
        // After we leave, we no longer have access to this household — route out
